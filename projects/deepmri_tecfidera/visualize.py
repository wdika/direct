...
def plot_reconstructions(loader, models, data_kwargs, mode='absolute', t_max=None, figsize=(30, 20), crop_box=None,
    ...
    for mask, etaloc in zip(data_kwargs['mask_path'][::2], map(int, data_kwargs['mask_path'][1::2])):
        ...
        if etaloc != -1:
            ...
            eta_mode_ssim = ' - SSIM:' + str(
                np.round(compare_ssim(target, eta_mode, data_range=target.max() - target.min()), 3)) + \
                            ' PSNR:' + str(
                np.round(compare_psnr(target, eta_mode), 1))

            plotdicts[0].update({gs[etaloc // grid[1], etaloc % grid[1]]: (
                eta_mode, '' if notitles else '{}x'.format(int(acc)) + eta_mode_ssim,
                (fsc_ax, np.squeeze(fourier_shells(batch['eta'].squeeze())), next(colorpalette)), cmaps[etaloc],
        ...
        plotdicts[0].update({gs[(etaloc + 1) // grid[1], (etaloc + 1) % grid[1]]: (pics_mode, 'CS' + pics_mode_ssim, (
            fsc_ax, np.squeeze(fourier_shells(batch['pics'].squeeze())), next(colorpalette)), cmaps[etaloc],
                                                                                   min_clims[etaloc],
                                                                                   max_clims[etaloc])})

        plotdicts[0].update({gs[(etaloc + 2) // grid[1], (etaloc + 2) % grid[1]]: (np.zeros_like(target), '', (
            fsc_ax, np.squeeze(fourier_shells(np.zeros_like(target).squeeze())), next(colorpalette)), cmaps[etaloc],
                                                                                   min_clims[etaloc],
                                                                                   max_clims[etaloc])})
            ...
            for img, pdict in zip(imgs, plotdicts[i:]):
                ...
                im_mode_ssim = ' - SSIM:' + str(
                    np.round(compare_ssim(target, im_mode, data_range=target.max() - target.min()), 3)) + \
                               ' PSNR:' + str(
                    np.round(compare_psnr(target, im_mode), 1))

                if error:
                    gl = 2 * gl
                    err = mode[gl](img_np - target_np)
                    pdict.update({gs[(gl + 1) // grid[1], (gl + 1) % grid[1]]: (
                        err, '' if notitles else 'Error: {}'.format(model.name), (None, None, None), cmaps[gl],
                        min_clims[gl], max_clims[gl])})

                pdict.update({gs[gl // grid[1], gl % grid[1]]: (
                    im_mode, '' if notitles else '{}'.format(model.name) + im_mode_ssim,
                    (fsc_ax, np.squeeze(fourier_shells(img.squeeze())), next(colorpalette)), cmaps[gl], min_clims[gl],
                    max_clims[gl])})
