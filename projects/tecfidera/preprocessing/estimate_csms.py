            ...
            for sense_ref_scan in sense_ref_scans:
                ...
                if name == '501':  # estimate csms from the sense ref scan
                    ...
                    input_sense_ref_scan_kspace = complex_tensor_to_complex_np(
                        input_sense_ref_scan.unsqueeze(-2).unsqueeze(1))#.permute(1, 2, 0, 3))  # readout dir, phase-encoding dir, slices, coils

                    coil_sensitivities_maps = []
                    for ref_kspace in range(input_sense_ref_scan_kspace.shape[0]):
                        coil_sensitivities_maps.append(bart(1, f"caldir {calibration_region_size}",
                                                            input_sense_ref_scan_kspace[0]))
                    ...
                    csm = T.ifftshift(torch.from_numpy(csm).squeeze(-2).squeeze(1), dim=(1, 2))
