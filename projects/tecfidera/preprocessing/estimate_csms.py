            ...
            for sense_ref_scan in sense_ref_scans:
                ...
                if name == '501':  # estimate csms from the sense ref scan
                    ...
                    input_sense_ref_scan_kspace = complex_tensor_to_complex_np(
                        input_sense_ref_scan.unsqueeze(1))  # readout dir, phase-encoding dir, slices, coils

                    coil_sensitivities_maps = []
                    for slice in range(input_sense_ref_scan_kspace.shape[0]):
                    ...
                    csm = T.ifftshift(torch.from_numpy(csm).squeeze(1), dim=(1, 2))
