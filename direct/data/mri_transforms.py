...
class CropAndMask(DirectClass):
    ...
    def __call__(self, sample: Dict[str, Any]):
        ...
        backprojected_masked_kspace = backprojected_masked_kspace * torch.conj(sample["sensitivity_map"])
        ...
        plt.subplot(1, 3, 2)
        plt.imshow(np.abs(T.tensor_to_complex_numpy(torch.sum(torch.conj(sample["sensitivity_map"]), dim="coil"))), cmap='gray')
        plt.subplot(1, 3, 3)
...
class EstimateBodyCoilImage(DirectClass):
...
class EstimateSensitivityMap(DirectClass):
    def __init__(
        self,
        kspace_key: str,
    ...
    def estimate_acs_image(self, sample):
        ...
        if self.gaussian_sigma == 0 or not self.gaussian_sigma:
            ...
            kspace_acs = (
                kspace_data * sample["acs_mask"] * gaussian_mask.align_as(kspace_data)
                + 0.0
            )
